...
function ReArrangeUnicodeText(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        if (i < str.length && IsBanglaPreKar(str.charAt(i))) {
            var j = 1;
            while (i - j >= 0 && IsBanglaBanjonborno(str.charAt(i - j))) {
...
export function convertUnicodeToBijoy(line) {
    if (!line) return "";
